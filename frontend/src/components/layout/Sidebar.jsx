...
import {
  ...
  FileBarChart, Calendar, Tag, ClipboardList, LogOut, ChevronRight, User, X
} from "lucide-react";
...
export default function Sidebar({ open, onClose }) {
  ...
  const handleNavClick = () => {
    // Close drawer on mobile when a nav link is clicked
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="fixed inset-y-0 left-0 z-40 w-64 flex flex-col bg-primary text-white border-r border-white/5
                   lg:translate-x-0 lg:animate-none"
        // On desktop, always visible (override the transform via CSS)
      ...
      {/* Desktop: static sidebar that's always shown via CSS (not affected by motion) */}
      <style>{`
        @media (min-width: 1024px) {
          /* Force sidebar visible on desktop regardless of JS state */
